...
int main( int argc, char** argv ) {
	...
	Field*		etaInt		= new Field( "eta-int", hMesh, 1, pbcs );
	...
	Field*		etaIntPrev	= new Field( "eta-int-prev", hMesh, 1, pbcs );
	Field*		velBarTemp	= new Field( "vel-bar-temp", vMesh, 2, vbcs );
	Field*		etaIntTemp	= new Field( "eta-int-temp", hMesh, 1, pbcs );
	...
	rl = new RigidLidEqn2( velTop, velBot, etaInt, velBar, presSurf, topography, &topParams, &botParams, dt, velTopPrev, velBotPrev );
	rl->Setup( 1 );

	/* initial guess for the top layer */
	tl = new TopLayerEqn( velTop, &topParams, dt );
	tl->Setup( 1 );
	tl->Solve( NULL, presSurf, etaInt );
	WriteXDMFHeader( 0 );
